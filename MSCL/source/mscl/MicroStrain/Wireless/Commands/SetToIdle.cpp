...
namespace mscl
{
	...
	void SetToIdle::Response::cancel()
	{
		...
		//any bytes sent to the base station when in Set to Idle mode
		//is considered canceling the Set to Idle command. 

		//get the original timeout
		uint64 origTimeout = m_baseStation.baseCommandsTimeout();

		//change the timeout to 1 millisecond so that we timeout quickly (the ping response will fail when in set to idle mode)
		m_baseStation.baseCommandsTimeout(1);

		try
		{
			//simply send the ping command to the base station (ignore the response)
			m_baseStation.ping();

			//reset the original timeout
			m_baseStation.baseCommandsTimeout(origTimeout);
		}
		catch(...)
		{
			//reset the original timeout
