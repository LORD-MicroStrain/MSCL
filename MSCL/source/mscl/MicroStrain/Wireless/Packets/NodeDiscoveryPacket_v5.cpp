...
namespace mscl
{
    const DeliveryStopFlags NodeDiscoveryPacket_v5::stopFlags_nodeDiscovery(true, false, false, false);

    bool NodeDiscoveryPacket_v5::integrityCheck(const WirelessPacket& packet)
    {
        const uint8 RADIO_CHANNEL_MIN = 11;
        const uint8 RADIO_CHANNEL_MAX = 26;

        //verify the payload size is correct
        if(packet.payload().size() != 27)
        {
            return false;
        }
        ...
        if(packet.type() != packetType_nodeDiscovery_v5)
        {
