...
namespace mscl
{
    ...
    void SyncSamplingMathPacket_aspp3::parseSweeps()
    {
        ...
        const uint8 NUM_CHANNEL_BYTES = static_cast<uint8>(payload.bytesRemaining() - 6);   //subtract node rssi, base rssi, and checksum bytes
    ...
    bool SyncSamplingMathPacket_aspp3::integrityCheck(const WirelessPacket& packet)
    {
        ...
        if(payload.size() < 27)
        {
            //payload must have at least 27 bytes to be valid
    ...
    UniqueWirelessPacketId SyncSamplingMathPacket_aspp3::getUniqueId(const WirelessPacket& packet)
    {
        static const std::size_t PAYLOAD_OFFSET_TICK = 9;
