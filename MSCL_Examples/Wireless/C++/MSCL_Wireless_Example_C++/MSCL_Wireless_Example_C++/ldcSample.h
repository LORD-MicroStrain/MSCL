...
void packetParser(mscl::BaseStation& base, mscl::WirelessNode& node) 
{
    ...
    for (int i = 0; i < 20; ++i)
    {
        ...
        {   // Everything available in a sweep 
            ...
            if(sweep.samplingType() == 0)           // Packet Type Sync 
                for (const auto& dp : sweep.data())
                {
                    std::cout << "  " << dp.channelName()
                        << " = " << dp.as_float() << std::endl;
                }
            else if(sweep.samplingType() == 6)     // Packet Type SHM
                for (const auto& dp : sweep.data())
                {
                    std::cout << "  " << dp.channelName()
                        << " = " << dp.as_StructuralHealth().damage() << std::endl;
                }
        }
