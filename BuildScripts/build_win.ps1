...
param (
    ...
    [String[]]$python3Versions,
    [String[]]$python3Dirs,
    [String[]]$python2Versions,
    [String[]]$python2Dirs
)
...
try
{
    ...
    cmake -S "${project_dir}" -B "${buildDir}" -G "${generator}" -A "${arch}" -T "${toolset}" `
        ...
        -DMSCL_PYTHON2_VERSIONS="${python2Versions}" `
        -DMSCL_BUILD_PYTHON3="ON" `
        -DMSCL_PYTHON3_VERSIONS="${python3Versions}" `
        ...
        -DMSCL_BRANCH="${branch}" `
        `
        -UPython3_ROOT -DPython3_ROOT="${python3Dirs}" `
        `
        -UPython2_ROOT -DPython2_ROOT="${python2Dirs}"
...
#
#     # Build python3
#     if ($python3Versions.Count)
#     {
#         foreach ($python3Dir in ${python3Versions}.split(","))
#         {
#             cmake -S "${project_dir}" -B "${buildDir}" -G "${generator}" -A "${arch}" -T "${toolset}" `
#                 -DCMAKE_VERBOSE_MAKEFILE="ON" `
#                 -DBUILD_SHARED_LIBS="OFF" `
